...
export async function loader({ request }: Route.LoaderArgs) {
  const { themeCookie } = await import("~/routes/api.theme");
  const raw    = await themeCookie.parse(request.headers.get("Cookie"));
  const accent = (raw?.accent as string) || "orange";
  return { accent };
}

export default function App({ loaderData }: Route.ComponentProps) {
  const { accent } = loaderData;

  return (
    <html lang="en" className="h-full" data-accent={accent} suppressHydrationWarning>
